...
export default class Triangle {
  ...
  calcNormal(isCcw?: boolean): vec3{
    let vertices = this.vertices;
    if (this.normal === undefined) {
      let directionA = vec3.subtract(vec3.create(), vertices[1].position, vertices[0].position);
      let directionB = vec3.subtract(vec3.create(), vertices[2].position, vertices[1].position);
      if (isCcw) {
