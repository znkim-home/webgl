...
const Tessellator_js_1 = __importDefault(require("@/assets/webgl/functional/Tessellator.js"));
const gl_matrix_1 = require("gl-matrix"); // eslint-disable-line no-unused-vars
class Polygon extends Renderable_js_1.default {
    ...
    getBuffer(gl) {
        if (this.buffer === undefined || this.dirty === true) {
            ...
            let bottomPositions = this.coordinates.map((coordinate) => gl_matrix_1.vec3.fromValues(coordinate[0], coordinate[1], 0));
            ...
            let topTriangles = Tessellator_js_1.default.tessellate(topPositions);
            let bottomTriangles = Tessellator_js_1.default.tessellate(bottomPositions, false);
    ...
    createSideTriangle(topPositions, bottomPositions, isCCW = true) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (isCCW) {
                triangles.push(new Triangle_1.default(topB, topA, bottomA));
                triangles.push(new Triangle_1.default(topB, bottomA, bottomB));
            }
            else {
                triangles.push(new Triangle_1.default(topB, bottomA, topA));
                triangles.push(new Triangle_1.default(topB, bottomB, bottomA));
            }
        }
        return triangles;
    }
