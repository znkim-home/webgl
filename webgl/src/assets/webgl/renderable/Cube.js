...
const TextureCoodinator_1 = __importDefault(require("../functional/TextureCoodinator"));
const gl_matrix_1 = require("gl-matrix"); // eslint-disable-line no-unused-vars
class Cube extends Renderable_js_1.default {
    constructor(options) {
        super();
        this.init(options);
    }
    init(options) {
        this.texturePosition = gl_matrix_1.vec2.fromValues(0, 0);
        this.size = gl_matrix_1.vec3.fromValues(128, 128, 128); // size : width, length, height
        ...
        if (options === null || options === void 0 ? void 0 : options.size)
            this.size = gl_matrix_1.vec3.set(this.size, options.size.width, options.size.length, options.size.height);
    ...
    getBuffer(gl) {
        if (this.buffer === undefined) {
            ...
            if (this.texture) {
                ...
                this.textureSize = gl_matrix_1.vec2.fromValues(512, 512);
            }
            let w = this.size[0] / 2;
            let l = this.size[1] / 2;
            let h = this.size[2];
            let color = this.color;
            let p0 = gl_matrix_1.vec3.fromValues(-w, -l, 0);
            let p1 = gl_matrix_1.vec3.fromValues(w, -l, 0);
            let p2 = gl_matrix_1.vec3.fromValues(w, l, 0);
            let p3 = gl_matrix_1.vec3.fromValues(-w, l, 0);
            let p4 = gl_matrix_1.vec3.fromValues(-w, -l, h);
            let p5 = gl_matrix_1.vec3.fromValues(w, -l, h);
            let p6 = gl_matrix_1.vec3.fromValues(w, l, h);
            let p7 = gl_matrix_1.vec3.fromValues(-w, l, h);
            ...
            let textureCoordinate = TextureCoodinator_1.default.calcTextureCoordinate(this.textureSize, this.texturePosition, 16);
